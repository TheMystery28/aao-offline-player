  ...
  function showScopeEditorModal(pluginFilename) {
    ...
    function resolveCollectionTitle(colId) {
      for (var i = 0; i < (cachedCollections || []).length; i++) {
        if (cachedCollections[i].id === colId) return cachedCollections[i].title;
      }
      return colId;
    }
    ...
    function resolveCaseTitle(caseId) {
      ...
      for (var i = 0; i < (cachedCases || []).length; i++) {
        if (cachedCases[i].case_id === id) return cachedCases[i].title;
      }
      return "Case " + caseId;
    }
    ...
    function refreshScopeEditor() {
      invoke("list_global_plugins").then(function (manifest) {
        ...
        globalLabel.style.cssText = "color:#ccc; font-size:0.9rem;";
        globalLabel.textContent = "Globally " + (globallyDisabled ? "disabled" : "enabled");
          ...
          for (var i = 0; i < scripts.length; i++) {
            (function (filename) {
              ...
              scopeBtn.addEventListener("click", (function (fn) {
                return function () { showScopeEditorModal(fn); };
              })(filename));
