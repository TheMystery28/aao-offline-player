...
Modules.load(new Object({
	name : 'player',
	dependencies : ['engine_events', 'input_manager', 'style_loader', 'trial', 'player_save', 'display_engine_screen', 'form_elements', 'language', 'nodes', 'page_loaded', 'events', 'player_sound', 'player_images', 'player_actions', 'actions_parameters', 'var_environments', 'player_courtrecord', 'expression_engine', 'player_debug', 'keyboard_controls', 'gamepad_controls'],
	init : function()
	{
        ...
        register_custom_function('current_frame_id', function(){
            return player_status.current_frame_id;
        });
	}
}));
...
function player_init()
{
	...
	else
	{
		...
		if('save_data' in _GET)
		{
			var barsExpected = 0;
			var barsDone = 0;
			var bars = document.querySelectorAll('#screen-loading .loading-bar');
			barsExpected = bars.length;
			function checkAutoStart() {
				barsDone++;
				if(barsDone >= barsExpected) {
					button_start.click();
				}
			}
			if(barsExpected === 0) {
				// No loading bars — just click start
				button_start.click();
			} else {
				for(var bi = 0; bi < bars.length; bi++) {
					registerEventHandler(bars[bi], 'loadComplete', checkAutoStart, false);
				}
			}
		}
	}
...
function readFrame(frame_index)
{
	...
	if(!frame_data)
	{
		...
		// Auto-continue: if in a collection with a next part, redirect instead of showing end screen.
		if(typeof tryGoToNextPart === 'function' && tryGoToNextPart())
		{
			return;
		}
		// No next part (standalone case or last part) — show end screen.
